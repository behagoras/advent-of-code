...
The Elves do have the manual for the machines, but the section detailing the initialization procedure was eaten by a [Shiba Inu](https://en.wikipedia.org/wiki/Shiba_Inu)![Attachment.tiff](Attachment.tiff). All that remains of the manual are some indicator light diagrams, button wiring schematics, and joltage requirements for each machine.
...
(To begin, [get your puzzle input](https://adventofcode.com/2025/day/10/input)![Attachment.tiff](Attachment.tiff).)
...
The Elves were right: getting the indicator lights set was only half the battle. The reactor still isn’t communicating, because each machine’s **joltage counters** must now be configured as well. The curly braces at the end of each machine’s line indicate the target values for the machine’s counters. For example, {3,5,4,7} means that counter 0 needs to reach 3, counter 1 needs to reach 5, counter 2 needs to reach 4, and counter 3 needs to reach 7. Initially, all counters start at 0.



Pressing a button now increments each of the listed counters by 1 (instead of toggling lights). For instance, in the first machine’s schematics, button (3) increases counter 3 each time it’s pressed, and button (1,3) increases counters 1 and 3 each time it’s pressed, and so on.



Determine the fewest total button presses required to correctly set all of the joltage counters on all of the machines to their target values.



(Although it hasn’t changed, you can still [get your puzzle input](https://adventofcode.com/2025/day/10/input)![Attachment.tiff](Attachment.tiff).)

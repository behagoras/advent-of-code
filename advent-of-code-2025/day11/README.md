...
(To begin, [get your puzzle input](https://adventofcode.com/2025/day/11/input)![Attachment.tiff](Attachment.tiff).)
...
Even after finding all those paths, the reactor still isn’t communicating. It turns out the problematic data path might involve certain critical devices. An Elf pulls up an updated list of connections and tells you to try a slightly different approach.



This time, consider the device labeled svr as the starting point instead of you. Additionally, the path from svr to out must pass through two specific devices: one labeled fft and one labeled dac. (The order in which data goes through fft and dac doesn’t matter, as long as both are included somewhere along the path.)



How many distinct paths are there from svr to out that go through **both** fft **and** dac?



(Although it hasn’t changed, you can still [get your puzzle input](https://adventofcode.com/2025/day/11/input)![Attachment.tiff](Attachment.tiff).)
